...
def fetch_yahoo(symbol: str = SYMBOL, interval: str = "1d", period_days: int = 400) -> list[Bar]:
    end = datetime.now(timezone.utc); start = end - timedelta(days=period_days)
    ...
    r.raise_for_status(); payload=r.json(); result=((payload.get("chart") or {}).get("result") or [])
    if not result: raise RuntimeError("Yahoo chart returned no data")
    item=result[0]; timestamps=item.get("timestamp") or []; quote=(item.get("indicators") or {}).get("quote",[{}])[0]; bars=[]
...
def classify_episode(criteria):
    ...
    if "FAILED_SPIKE" in s: return "FAILED_SPIKE"
    if "VOLUME_5X" in s and ("DAILY_UP_20" in s or "INTRADAY_RANGE_30" in s): return "VOLUME_MOMENTUM_SPIKE"
    if "DAILY_DOWN_20" in s: return "MAJOR_DOWNSIDE"
    if "DAILY_UP_20" in s: return "MAJOR_UPSIDE"
...
def detect_daily_episodes(bars):
    ...
    for i,b in enumerate(bars):
        if i==0: continue
        prev=bars[i-1]; close_change=(b.close/prev.close-1)*100 if prev.close else 0; intraday=(b.high/b.low-1)*100 if b.low else 0; rvol=_rvol(bars[:i+1],20); criteria=[]
        ...
        high_from_open=(b.high/b.open-1)*100 if b.open else 0; close_from_high=(b.close/b.high-1)*100 if b.high else 0
        if high_from_open>=20 and close_from_high<=-15: criteria.append("FAILED_SPIKE")
        if criteria: episodes.append({"date":b.ts.date().isoformat(),"open":b.open,"high":b.high,"low":b.low,"close":b.close,"previous_close":prev.close,"close_change_pct":round(close_change,4),"intraday_range_pct":round(intraday,4),"volume":int(b.volume),"rvol_20d":round(rvol,4),"criteria":criteria,"classification":classify_episode(criteria)})
    return episodes
...
def should_alert(signal,state):
    ...
    state["last_key"]=key; return True
