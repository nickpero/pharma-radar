...
def build_daily_event_study(bars, episodes):
    ...
    for ep in episodes:
        ...
        for w in [-10,-5,-3,-2,-1,0,1,3,5]:
            j=i+w
            if 0<=j<len(bars):
                b=bars[j]; base=bars[i-1].close if i>0 else None
                row["windows"][str(w)]={"close":b.close,"volume":b.volume,"return_vs_prior_close":_pct(b.close,bars[j-1].close) if j>0 else None,"return_vs_event_prev_close":_pct(b.close,base)}
        studies.append(row)
    return studies

def run():
    bars=fetch_yahoo(SYMBOL,"1d",420); episodes=detect_daily_episodes(bars); studies=build_daily_event_study(bars,episodes)
    OUT.mkdir(parents=True,exist_ok=True)
    payload={"ticker":SYMBOL,"windows_intraday_minutes":WINDOWS,"daily_event_studies":studies,"note":"Intraday pre-spike study is populated when sufficiently long historical 5m data is available; free endpoints may restrict older intraday history."}
